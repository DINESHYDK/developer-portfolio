...
const fetchLeetCodeStats: PlatformFetcher = async (username) => {
  ...
  const platform: CodingPlatform = {
    ...
    rating: contestRaw?.rating ? Math.round(contestRaw.rating) : undefined,
    globalRanking: contestRaw?.globalRanking ?? statsRaw.ranking,
    contestsAttended: contestRaw?.attendedContestsCount ?? 0,
    topPercentage: contestRaw?.topPercentage ?? undefined,
    breakdown: [
